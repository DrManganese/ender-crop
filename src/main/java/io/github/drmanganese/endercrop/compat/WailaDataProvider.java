        ...
        if (accessor.getBlock() instanceof BlockCropEnder) {
            if (accessor.getWorld().getBlockState(accessor.getPosition().down()).getBlock() == Blocks.FARMLAND) {
                if (accessor.getMetadata() / 7.0F < 1.0F) {
                    ...
                    if (light > 7) {
                        currenttip.clear();
                        currenttip.add(0, TextFormatting.RED + I18n.format("endercrop.waila.nogrowth") + " -> " + I18n.format("endercrop.waila.light") + " : " + light + TextFormatting.ITALIC + " (>7)");
                    }
                }
            }
            ...
            if (accessor.getBlockState().getValue(BlockTilledEndStone.MOISTURE) == 7) {
                currenttip.add(TextFormatting.ITALIC + I18n.format("endercrop.waila.moist"));
            } else {
                currenttip.add(TextFormatting.ITALIC + I18n.format("endercrop.waila.dry"));
            }
